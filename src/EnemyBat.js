...
/*    for (i = 0; i < 7; i++) {　　　　　　
      for (j = 0; j < 10; j++) {
        if (level[i][j] == 5) {
        */
          this.setPosition(100,100/*tileSize / 2 + tileSize * j, 96 * (7 - i) - tileSize / 2*/);
/*        }
      }
    }
*/
  ...
  update: function(dt) {
    ...
    var playerBoundingBox = playerSprite.getBoundingBox();
        var batBoundingBox = this.getBoundingBox();
        //rectIntersectsRectは２つの矩形が交わっているかチェックする
        if (cc.rectIntersectsRect(playerBoundingBox, batBoundingBox) /*&& playerSprite.invulnerability == 0*/ ) {
          ...
          playerSprite.invulnerability = 300;
    ...
    if (this.FrameCount % 4 == 0) {
      ...
      var offset_x = playerSprite.getPosition().x - this.getPosition().x;
      var offset_y = playerSprite.getPosition().y - this.getPosition().y;
      ...
      if (this.getPosition().y < playerSprite.y + 20) velocity_y += 0.05;
      //8の字旋回軌道をsin計算で適当に補正
